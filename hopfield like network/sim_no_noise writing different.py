...
def reset_network_inh(unit_list,nb_units):

    for i in range(nb_units):

        for l in range(len( unit_list[i].left_neuron.inh_syns)):
            unit_list[i].left_neuron.inh_syns[l].w = 0
        
        for r in range(len( unit_list[i].right_neuron.inh_syns)):
            unit_list[i].right_neuron.inh_syns[r].w = 0
...
def main():
    ...
    unit_list_2 = making_network(nb_units)
    ...
    #meta_state_list = []
    nb_iter= 250
    nb_iter_rewriting = 10
    distance_to_writed=np.zeros((nb_writed_states,nb_iter))

    for k in range(nb_iter_rewriting):
        print(k)
        for i in range(nb_iter):
            ...
            #     distance_to_writed[k,j,i]=min(dist_to_inverse, dist)
            ...
            change_all_states(unit_list_2, get_state(unit_list))
            pot_state_exc_mult(unit_list_2, exc_write/(nb_iter_rewriting*3))

        reset_network_inh(unit_list,nb_units)
    ...
    for i in range(nb_iter):
        ...
        for j in range(nb_writed_states):
            writed_state = writed_state_list[j]
            dist_to_inverse = np.count_nonzero(get_state(unit_list_2) != np.where(writed_state == 1, 0, 1))
            dist = np.count_nonzero(get_state(unit_list_2) != writed_state)
            distance_to_writed[j,i]=min(dist_to_inverse, dist)

        state = np.random.randint(0, 2, nb_units)
        change_all_states(unit_list_2, state)
        for u in unit_list_2:
            u.decide_state()
        #print_state(unit_list_2,nb_row)

        pot_state_inh(unit_list_2, inh_pot)
